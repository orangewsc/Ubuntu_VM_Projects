...
int UDPInit()
{
	...
	request_addr.sin_port = htons(3333);
	request_addr.sin_addr.s_addr = inet_addr("192.168.2.10");   //windows:servaddr.sin_addr.S_un.S_addr  unix:servaddr.sin_addr.s_addr = inet_addr("172.31.1.48);


    response_addr.sin_family = AF_INET;
	response_addr.sin_port = htons(4444);
	response_addr.sin_addr.s_addr = inet_addr("192.168.2.1");   //windows:servaddr.sin_addr.S_un.S_addr  unix:servaddr.sin_addr.s_addr = inet_addr("172.31.1.48);
    
    local_addr.sin_family = AF_INET;
	local_addr.sin_port = htons(2222);
	local_addr.sin_addr.s_addr = inet_addr("192.168.2.10");   //windows:servaddr.sin_addr.S_un.S_addr  unix:servaddr.sin_addr.s_addr = inet_addr("172.31.1.48);
...
int Sendto(const char* sendbuf)
{
	...
	{
		sleep(1);
		if(sendto(socket_client,sendbuf,10,0,(sockaddr*)&response_addr, sizeof(response_addr))<0)
		{
...
int Recvfrom(char recvbuf[56])
{
	int len=sizeof(sockaddr_in);
	while(true)
	{
		sleep(1);
		if(recvfrom(socket_server,recvbuf,56,0,(sockaddr*)&remote_addr,(socklen_t*)&len)<0)
		{
